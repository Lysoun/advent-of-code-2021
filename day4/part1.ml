...
let find_winning_board boardsList =
    ...
    let rec aux numbers lastNumberCalled boards =
        let winningsBoards = (List.filter (is_winning_board) boards) in
        if ((List.length winningsBoards) >= 1)
        then ((List.nth winningsBoards 0), lastNumberCalled)
        else (
            let calledNumber = (List.hd numbers) in
            (mark_boards boards calledNumber);
            (aux (List.tl numbers) calledNumber boards)
        )
    in
...
let rec input_lines currentBoard lineNumber boardsList =
   match try (input_line file) with End_of_file -> "EOF" with
      | "EOF" -> (find_winning_board (currentBoard:: boardsList))
