...
let find_step_where_all_flash_simultaneously dumboOctopuses =
    let rec find_step_where_all_flash_simultaneously_aux step dumboOctopuses =
        let (dumboOctopusesAfterStep, stepFlashesNumber) = (execute_step (increase_all_octopuses dumboOctopuses)) in
        if stepFlashesNumber = 100
        then step
        else (find_step_where_all_flash_simultaneously_aux (step + 1) dumboOctopusesAfterStep)
    in

    (find_step_where_all_flash_simultaneously_aux 1 dumboOctopuses)
;;
...
let rec input_lines file dumboOctopuses lineNumber =
   match try (input_line file) with End_of_file -> "" with
      | "" -> (find_step_where_all_flash_simultaneously dumboOctopuses)
      | line ->
